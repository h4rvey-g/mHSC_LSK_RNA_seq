...
# extract lines from all data/06.rMATs/${group}/summary/${AS_type}_*.tsv, which "GeneID" matches the gene_list, only keep column "GeneID", "geneSymbol", "PValue", "FDR", "IncLevelDifference", save all results to results/06_2.key_genes/LSK_AS_result.tsv
echo "" >results/06_2.key_genes/LSK_AS_result.tsv
get_AS_result() {
    gene_list=(
    ...
    # save the result to results/06_2.key_genes/LSK_AS_result.tsv
    cat $tmp_file.tmp >>results/06_2.key_genes/LSK_AS_result.tsv
}

export -f get_AS_result
parallel -j 30 get_AS_result ::: ${group[@]} ::: ${AS_type[@]}
sort results/06_2.key_genes/LSK_AS_result.tsv | uniq >results/06_2.key_genes/unique_LSK_AS_result.tsv
mv results/06_2.key_genes/unique_LSK_AS_result.tsv results/06_2.key_genes/LSK_AS_result.tsv
# delete the tmp files
rm results/06_2.key_genes/*tmp*
# # File path
# file="results/06_2.key_genes/LSK_AS_result.tsv"
...
find results/06_2.key_genes -type f -not -path "*index*" | zip results/LSK_key_genes_AS.zip -@
