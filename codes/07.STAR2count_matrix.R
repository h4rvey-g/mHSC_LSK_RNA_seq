...
packages <- c("BiocManager", "GenomicFeatures", "DGEobj.utils", "ggrepel", "pheatmap")
...
for (compare in compare_list) {
 ...
 print(res) 
  ...
  for (logFC_t in c(round(logFC_t, 2), 2)) {
    ...
    data <- na.omit(dat)
    group2 <- data.frame(group=group_list[BA])
    rownames(group2)=colnames(dat)
