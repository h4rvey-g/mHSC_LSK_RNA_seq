...
# need to remove the quotation mark around '45' (2 places) in /opt/conda/lib/python3.9/site-packages/deeptools/correlation.py
mkdir -p /mHSC_RNA_seq/results/05.bam_coverage
# index all bam files first
for bamfile in /mHSC_RNA_seq/data/03.STARalign/*/*.bam; do
    if [ ! -f "${bamfile}.bai" ]; then
        samtools index "${bamfile}"
    fi
done
# use multiBamSummary to get the coverage of all bam files in /mHSC_RNA_seq/data/03.STARalign
multiBamSummary bins \
    --bamfiles /mHSC_RNA_seq/data/03.STARalign/*/*.bam \
    --labels $(ls /mHSC_RNA_seq/data/03.STARalign/*/*.bam | xargs -n 1 basename | sed 's/Aligned\.sortedByCoord\.out\.bam$//') \
